//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Treatment.Entity
{
    using System;
    using System.Collections.Generic;
    
    public partial class Treatment_Status
    {
        public Treatment_Status()
        {
            this.Treatment_Master = new HashSet<Treatment_Master>();
        }
    
        public int Treatment_Status_Id { get; set; }
        public string Treatment_Status_Name_Ar { get; set; }
        public string Treatment_Status_Name_En { get; set; }
    
        public virtual ICollection<Treatment_Master> Treatment_Master { get; set; }
    }
}
